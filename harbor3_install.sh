...
#替换变量
sed -i "s/reg.mydomain.com/$harbor_host3/g" harbor.cfg | grep ^hostname
...
openssl req -x509 -new -nodes -key server.key -subj "/CN=$harbor_host3" -days 7120 -out server.crt
cd -
./install.sh
